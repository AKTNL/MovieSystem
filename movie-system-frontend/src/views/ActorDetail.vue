    ...
    <div style="max-width: 1000px; margin: 20px auto;">
        <el-card shadow="never">
            <div style="display: flex;">
                <!-- 左侧头像 -->
                <el-image :src="actor.avatarUrl" style="width: 200px; height: 280px; border-radius: 8px;" fit="cover" />
                
                <!-- 右侧信息 -->
                <div style="margin-left: 30px; flex: 1;">
                    <h1 style="margin-top: 0;">{{ actor.name }}</h1>
                    <div style="color: #666; line-height: 2;">
                        <p><strong>性别：</strong> {{ actor.gender === 'M' ? '男' : '女' }}</p>
                        <p><strong>国籍：</strong> {{ actor.nationality }}</p>
                        <p><strong>出生日期：</strong> {{ actor.birthDate || '暂无' }}</p>
                    </div>
                </div>
            </div>
        </el-card>
        ...
        <div style="margin-top: 30px;">
            <h3 style="border-left: 4px solid #409EFF; padding-left: 10px;">参演作品</h3>
            <el-row :gutter="20">
